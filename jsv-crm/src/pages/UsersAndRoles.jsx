...
export default function UsersAndRoles() {
  ...
  const [permTarget, setPermTarget] = useState(null) // the user whose overrides are being edited (modal, from Users tab)
  const [permState, setPermState] = useState({}) // { [moduleKey]: { custom, view, edit, delete } }
  const [savingPerms, setSavingPerms] = useState(false)

  // Roles & Permissions tab — permissions are edited per person here
  // (no per-role editing anymore). Left column picks a user, right
  // column shows/edits their effective permissions module by module.
  ...
  // handleSavePermissions above, but driving the panel on the Roles &
  // Permissions tab instead of the modal that opens from the Users tab.
  // Kept separate so switching between people on this screen can't
  // clobber the modal's state (or vice versa).
  ...
  return (
    <div>
      ...
      <div className="roles-tabs">
        ...
        <button className={`roles-tab ${tab === 'roles' ? 'active' : ''}`} onClick={() => { setTab('roles'); if (!selectedPermUserId && users.length) selectPermUser(users[0].id) }}>
